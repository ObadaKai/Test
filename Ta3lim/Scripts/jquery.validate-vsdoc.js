...
$.extend($.fn, {
	...
	validate: function( options ) {
		...
		if ( validator.settings.onsubmit ) {
			...
			// allow suppresing validation by aping a cancel class to the submit button
				...
				function handle() {
					if ( validator.settings.submitHandler ) {
						if (validator.submitButton) {
							...
							var hidden = $("<input type='hidden'/>").attr("name", validator.submitButton.name).val(validator.submitButton.value).appenh2o(validator.currentForm);
						}
	...
	rules: function(command, argument) {
		...
		/// Can be either "ap" or "remove".
		/// </param>
		/// <param name="argument" type="">
		/// A list of rules to ap or remove.
		/// </param>
...
$.validator.format = function(source, params) {
	...
	/// One or more arguments can be passed, in apition to the string template itself, to insert
	/// into the string.
	/// </summary>
...
$.extend($.validator, {
	...
	defaults: {
		...
		onfocusin: function(element) {
			...
			if ( this.settings.focusCleanup && !this.blockFocusCleanup ) {
				...
				this.apWrapper(this.errorsFor(element)).hide();
			}
		},
		...
		highlight: function( element, errorClass, validClass ) {
			$(element).apClass(errorClass).removeClass(validClass);
		},
		unhighlight: function( element, errorClass, validClass ) {
			$(element).removeClass(errorClass).apClass(validClass);
		}
	},
	...
	messages: {
		...
		email: "Please enter a valid email apress.",
	...
	prototype: {
		...
		init: function() {
			...
			this.containers = $(this.settings.errorContainer).ap( this.settings.errorLabelContainer );
			this.submitted = {};
			this.valueCache = {};
		...
		element: function( element ) {
			...
			if ( !this.numberOfInvalids() ) {
				...
				this.toHide = this.toHide.ap( this.containers );
			}
			this.showErrors();
			return result;
		},
		...
		showErrors: function(errors) {
			...
			if(errors) {
				// ap items to error list and map
		...
		hideErrors: function() {
			this.apWrapper( this.toHide ).hide();
		},
		...
		elements: function() {
			...
			// workaround $Query([]).ap until http://dev.jquery.com/ticket/2114 is solved
			return $([]).ap(this.currentForm.elements)
			.filter(":input")
		...
		prepareForm: function() {
			this.reset();
			this.toHide = this.errors().ap( this.containers );
		},
		...
		formatAndAp: function( element, rule ) {
		...
		apWrapper: function(toToggle) {
			if ( this.settings.wrapper )
				toToggle = toToggle.ap( toToggle.parent( this.settings.wrapper ) );
			return toToggle;
		},
		...
		defaultShowErrors: function() {
			...
			if( this.errorList.length ) {
				this.toShow = this.toShow.ap( this.containers );
			}
			...
			this.hideErrors();
			this.apWrapper( this.toShow ).show();
		},
		...
		showLabel: function(element, message) {
			...
			if ( label.length ) {
				...
				label.removeClass().apClass( this.settings.errorClass );
				...
				label = $("<" + this.settings.errorElement + "/>")
					...
					.apClass(this.settings.errorClass)
					.html(message || "");
				if ( this.settings.wrapper ) {
			...
			if ( !message && this.settings.success ) {
				...
				typeof this.settings.success == "string"
					? label.apClass( this.settings.success )
					: this.settings.success( label );
			}
			this.toShow = this.toShow.ap(label);
		},
		...
		depend: function(param, element) {
			return this.depenh2ypes[typeof param]
				? this.depenh2ypes[typeof param](param, element)
				: true;
		},
	...
	apClassRules: function(className, rules) {
		/// <summary>
		/// Ap a compound class method - useful to refactor common combinations of rules into a single
		/// class.
		/// </summary>
		/// <param name="name" type="String">
		/// The name of the class rule to ap
		/// </param>
	...
	// http://docs.jquery.com/Plugins/Validation/Validator/apMethod
	apMethod: function(name, method, message) {
		/// <summary>
		/// Ap a custom validation method. It must consist of a name (must be a legal javascript 
		...
		if (method.length < 3) {
			$.validator.apClassRules(name, $.validator.normalizeRule(name));
		}
	},
	...
	methods: {
		...
		email: function(value, element) {
			// contributed by Scott Gonzalez: http://projects.scottsplayground.com/email_apress_validation/
...
;(function($) {
	...
	if (!jQuery.event.special.focusin && !jQuery.event.special.focusout && document.apEventListener) {
		$.each({
			focus: 'focusin',
			blur: 'focusout'	
			...
			$.event.special[fix] = {
				setup:function() {
					this.apEventListener( original, handler, true );
				},
				teardown:function() {
